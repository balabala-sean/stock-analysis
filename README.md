...
- tdx数据的实时/历史拉取，（无需安装客户端）

- buy signal : 基于均值回归策略的实现（可以自行优化）

- 后台静默运行，实时email进行信号的触达

- 5分钟开启量化交易（中低频，提供的策略相对轻量级，券商柜台的交易/仓位管理，可以自行开发）
...
| 名称             | 说明                                                                                                                                                          |
|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **通达信 (数据源)**  | 国内最主流的股票交易行情软件。提供实时行情（A股/港股/期货）、历史K线、技术指标分析、交易下单功能。本项目通过其免费的数据接口来获取股票行情。(相较akshare/yhfinance，通达信在数据质量/网络传输上都更加稳定可靠，可以达到商用的程度，量化专业版也可以采用QMT/JoinQuant等付费解决方案 |
...
| **买点策略**       | 买点信号策略，通过两个维度判断买入时机：通过价格的均值回归来计算顶底区间，通过慢速趋势指标增强判断转折点，满足条件时产生买入信号。 均值回归是主流的买点策略实现方案                                                                          |
| **卖点策略**       | 本项目未提供卖点策略，卖点在量化交易领域存在非一致性的特征（业内难题），有较多的个性化因素。建议：可以尝试均值回归的值大于90进行卖点的自定义（非最佳的模式，能达到可用，但还存在较大优化空间）。                                                           |
...
修改 `examples/single_stock_analysis.py` 底部参数可自定义股票代码、名称、周期等。
...
4. 如果 `email.enabled=true` 且触发买点，发送邮件通知
