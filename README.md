...
| **买点策略**       | 买点信号策略，通过两个维度判断买入时机：通过价格的均值回归来计算顶底区间，通过慢速趋势指标增强判断转折点，满足条件时产生买入信号。 均值回归是目前A股市场一种主流的买点策略（实现方案上大同小异）                                                                          |
...
#### 信号输出类型（⚠️⚠️⚠️⚠️⚠️重要⚠️⚠️⚠️⚠️⚠️）

批量分析完成后，会输出两种类型的信号：

| 信号类型 | 判断条件 | 说明 |
|---------|---------|------|
| **买点信号** | `FILTER_BUY = True` | 当前触发买入信号，满足策略条件，建议重点关注 |
| **低位关注信号** | `UP_LINE < 15` 且 `DOWN_LINE < 15` | 价格处于低位区间，未来可能产生买点信号，建议提前关注 |
